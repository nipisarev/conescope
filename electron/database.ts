...
class DatabaseService {
  ...
  private migrate(): void {
    ...
    try {
      this.db.exec(
        "ALTER TABLE instances ADD COLUMN type TEXT NOT NULL DEFAULT 'project'",
      );
    ...
    const colInfo = this.db.pragma("table_info(instances)") as {
      name: string;
      notnull: number;
    }[];
  ...
  getAllInstances(): any[] {
    ...
    return this.db
      .prepare(
        "SELECT * FROM instances WHERE ended_at IS NULL ORDER BY started_at ASC",
      )
      .all();
  }
  ...
  updateInstance(id: string, updates: any): void {
    ...
    const values = [...Object.values(updates), id];
    this.db
      .prepare(`UPDATE instances SET ${fields} WHERE id = ?`)
      .run(...values);
  }
  ...
  saveTerminalHistory(id: string, history: string[]): void {
    ...
    this.db
      .prepare("UPDATE instances SET terminal_history = ? WHERE id = ?")
      .run(json, id);
  }
  ...
  getTerminalHistory(id: string): string[] {
    if (!this.db) throw new Error("Database not initialized");
    const row = this.db
      .prepare("SELECT terminal_history FROM instances WHERE id = ?")
      .get(id) as { terminal_history: string | null } | undefined;
